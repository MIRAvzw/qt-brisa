...
class BRISA_UPNP_EXPORT BrisaServiceParserContext
{
    public:
        ...
        inline void setAction(BrisaAction *action) { this->action = action; }
        inline void setService(BrisaAbstractService *service) { this->service = service; }
        inline void setStateVariable(BrisaStateVariable *stateVariable) { this->stateVariable = stateVariable; }
        inline void setArgument(BrisaArgument *argument) { this->argument = argument; }

        inline bool hasParent(void) { return (parent != 0); }
        inline BrisaServiceParserContext *getParent(void) { return parent; }
